/*
 * @lc app=leetcode.cn id=1826 lang=cpp
 *
 * [1826] 有缺陷的传感器
 *
 * https://leetcode-cn.com/problems/faulty-sensor/description/
 *
 * algorithms
 * Easy (45.92%)
 * Likes:    2
 ...
 * 
 * 
 * 举个例子, 如果正确的数据是 [1,2,3,4,5] ， 此时 3 被丢弃了, 传感器会返回 [1,2,4,5,7] (最后的位置可以是任何值,
 * 不仅仅是 7).
 * 
 * 
 * 可以确定的是，最多有一个 传感器有缺陷。请返回这个有缺陷的传感器的编号 （1 或 2）。如果任一传感器 没有缺陷 ，或者 无法
 * 确定有缺陷的传感器，则返回 -1 。
 * 
 * 
 * 
