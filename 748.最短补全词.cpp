    ...
    string shortestCompletingWord(string licensePlate, vector<string> &words)
    {
        ...
        for (int j = 0; j < words.size(); j++)
        {
            for (int i = 0; i < licensePlate.size(); i++)
            {
                if (isalpha(licensePlate[i]))
                {
                    int temp = count(licensePlate.begin(), licensePlate.end(), licensePlate[i]);
                    if (count(words[j].begin(), words[j].end(), licensePlate[i]) < temp)
                        break;
                }
                if (i == licensePlate.size() - 1)
                    res.emplace_back(words[j]);
                if (words[j] == "in")
                {
                    cout << j << endl;
                }
